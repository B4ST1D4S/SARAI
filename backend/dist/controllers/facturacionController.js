...
// ─────────────────────────────────────────────────────────────
export async function crearIngresoYCuentaDesdeCita(citaId) {
...
const CAMPOS_RIPS_FECHA = ['fechaAtencion', 'fechaIngreso', 'fechaSalida'];
function extraerCamposRips(body) {
    const data = {};
    for (const campo of CAMPOS_RIPS_TEXTO) {
        if (body[campo] !== undefined)
            data[campo] = norm(body[campo]) || null;
    }
...
export async function addCuentaItem(req, res) {
    try {
        ...
        if (cargoId) {
            ...
            if (!codigo)
                codigo = cargo.cupsCodigoStr || cargo.codigo;
...
export async function updateCuentaItem(req, res) {
    try {
        ...
        if (req.body.departamento !== undefined)
            data.departamento = norm(req.body.departamento) || null;
        if (req.body.tipoRips !== undefined)
            data.tipoRips = norm(req.body.tipoRips) || null;
...
export async function deleteCuentaItem(req, res) {
    try {
        const item = await prisma.cuentaItem.findUnique({
            where: { id: req.params.itemId },
            include: { cuenta: true },
...
export async function buscarCargos(req, res) {
    try {
        const search = norm(req.query.search);
        const where = { activo: true, esFacturable: true };
        if (search) {
...
// ─────────────────────────────────────────────────────────────
export async function validarRips(req, res) {
    try {
...
//  FACTURAS
// ─────────────────────────────────────────────────────────────
export async function facturarCuenta(req, res) {
    try {
        const cuentaId = req.params.id;
        const cuenta = await prisma.cuenta.findUnique({
            where: { id: cuentaId },
        ...
        if (!cuenta)
            return res.status(404).json({ error: 'Cuenta no encontrada' });
        if (cuenta.factura)
            return res.status(409).json({ error: 'La cuenta ya fue facturada' });
        if (cuenta.estado !== 'ABIERTA')
            return res.status(409).json({ error: 'La cuenta no está abierta' });
