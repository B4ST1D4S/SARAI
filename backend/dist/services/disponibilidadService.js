...
// ─── DISPONIBILIDAD ────────────────────────────────────────────────────────
export async function getDisponibilidadMedico(medicoId) {
...
export async function updateDisponibilidad(id, data) {
    const { medicoId, fechaDesde, fechaHasta, ...rest } = data;
    const updateData = { ...rest };
    if (medicoId)
        updateData.medico = { connect: { id: medicoId } };
    if (fechaDesde !== undefined)
        updateData.fechaDesde = fechaDesde ? new Date(fechaDesde) : null;
    if (fechaHasta !== undefined)
        updateData.fechaHasta = fechaHasta ? new Date(fechaHasta) : null;
...
export async function deleteDisponibilidad(id) {
    ...
    const conCitas = citasFuturas.filter(c => {
        ...
        if (d.getDay() !== franja.diaSemana)
            return false;
        const minCita = d.getHours() * 60 + d.getMinutes();
        return minCita >= minIni && minCita < minFin;
    });
    if (conCitas.length > 0) {
        throw new Error(`No se puede eliminar: hay ${conCitas.length} cita${conCitas.length !== 1 ? 's' : ''} activa${conCitas.length !== 1 ? 's' : ''} en esta franja`);
    }
    return prisma.disponibilidadMedico.update({ where: { id }, data: { activo: false } });
}
// ─── BLOQUEOS ──────────────────────────────────────────────────────────────
export async function getBloqueos(medicoId) {
...
export async function getDisponibilidadesConCitas(medicoId) {
 ...
 */
export async function getMedicosPorTipoConsulta(_tipoConsultaNombre) {
 ...
 */
export async function getDiasDisponibles(medicoId, mes, // 1-12
anio, duracionOverride) {
    ...
    for (let dia = 1; dia <= diasEnMes; dia++) {
        ...
        if (!diasConFranja.has(diaSemana))
            continue;
        // ¿Hay bloqueo ese día?
        const bloqueado = bloqueos.some(b => new Date(b.fechaInicio) <= new Date(fechaStr + 'T23:59:59Z') &&
        ...
        if (tieneLibre)
            disponibles.push(dia);
    }
    return disponibles;
}
export async function getSlotsConEstado(medicoId, fecha, duracionOverride) {
    ...
    const ocupadas = new Set(citasDelDia.map((c) => {
...
export async function getSlotsDisponibles(medicoId, fecha, duracionOverride) {
