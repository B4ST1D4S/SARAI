...
import { createPaciente, getPacienteById, getAllPacientes, updatePaciente, deletePaciente, searchPacientes, } from '../services/pacientesService.js';
export async function create(req, res) {
    try {
        ...
        const { numeroDocumento, tipoDocumento, nombreCompleto, fechaNacimiento, genero, telefonos, email, whatsapp, direccion, ciudad, } = req.body;
...
export async function deletePac(req, res) {
    try {
        const { id } = req.params;
        if (!id) {
            res.status(400).json({ error: 'ID requerido' });
...
export async function verificarDuplicados(req, res) {
    try {
        ...
        const mismoNumeroOtroTipo = numero
            ? await prisma.paciente.findMany({
                where: {
                    ...
                    ...(tipo ? { tipoDocumento: { not: tipo } } : {}),
                },
                select: campoBase,
            })
            : [];
        // 2. Mismo nombre completo (búsqueda insensible a mayúsculas) → posible homónimo
        const mismoNombre = nombre && typeof nombre === 'string' && nombre.trim().length >= 4
            ? await prisma.paciente.findMany({
                where: {
                    nombreCompleto: {
