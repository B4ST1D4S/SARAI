...
export async function cargaMasiva(req: Request, res: Response): Promise<void> {
  try {
    ...
    const idx = (campo: string) => encabezados.indexOf(campo);

    const creados: string[] = [];
    const errores: { fila: number; username: string; error: string }[] = [];

    for (let i = 1; i < lineas.length; i++) {
      const cols = lineas[i].split(',').map(c => c.trim().replace(/^"|"$/g, ''));
      const get = (...campos: string[]) => {
