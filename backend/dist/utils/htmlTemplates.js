...
function fmtFecha(v, soloFecha = false) {
    if (!v)
        return '';
    try {
...
function fmtTel(telefonos) {
    if (!telefonos)
        return '';
    if (Array.isArray(telefonos))
        return telefonos.map((t) => t.numero ?? t).join(' / ');
...
// Encabezado empresa
// ═══════════════════════════════════════════════════════════════
function buildEmpresaHeader(medico, titulo, historia, clinica) {
...
// HISTORIA CLÍNICA
// ═══════════════════════════════════════════════════════════════
export function buildHCHtml(historia, clinica) {
  ...
  <table class="hc-table">
    ...
    <tr>
      <td style="vertical-align:top; padding-top:3px;">
        ...
        <div style="margin-top:3px; font-size:7.5px; color:#005B8E; font-weight:bold;">${med.nombre ? `${med.nombre} ${med.apellido ?? ''}` : ''}</div>
      </td>
      <td>
...
// ÓRDENES MÉDICAS
// ═══════════════════════════════════════════════════════════════
export function buildOrdenesHtml(historia, clinica) {
    const raw = historia.datosExtendidos ?? historia.contenido ?? {};
    const ext = typeof raw === 'string' ? JSON.parse(raw) : raw;
    ...
    const recoms = ordenes.recomendaciones ?? [];
    const pac = historia.paciente ?? {};
    const med = historia.usuario ?? {};
...
<body>
  ...
  <table class="hc-table">
    ...
    ${recoms.map((x) => `
    <tr><td class="cell-multi">${typeof x === 'string' ? x : (x.descripcion ?? x.texto ?? JSON.stringify(x))}</td></tr>`).join('')}
  </table>` : ''}
