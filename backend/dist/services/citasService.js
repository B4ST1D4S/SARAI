...
export async function createCita(data) {
    try {
        ...
        sendCitaConfirmation(cita).catch((err) => {
...
export async function getCitasByMedico(medicoId, estado, fechaInicio, fechaFin) {
    try {
        const where = {};
        if (medicoId)
        ...
        if (fechaInicio || fechaFin) {
            ...
            if (fechaFin)
                where.fechaHora.lte = new Date(fechaFin);
        }
...
export async function updateCita(citaId, data) {
    try {
...
export async function confirmarAsistencia(citaId) {
    try {
        const cita = await prisma.cita.update({
            where: { id: citaId },
            data: {
...
export async function completarCita(citaId, notas) {
    try {
        const cita = await prisma.cita.update({
            where: { id: citaId },
            data: {
...
async function sendCitaConfirmation(cita) {
    try {
...
async function sendCitaCancellation(cita) {
    try {
        const mailOptions = {
            from: process.env.EMAIL_USER || 'noreply@estegia.com',
            to: cita.paciente.email,
