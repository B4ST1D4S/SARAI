...
const DIAS_ESQUEMA: Record<string, number[]> = {
  LMV: [1, 3, 5],
  MJS: [2, 4, 6],
};

function tieneJornadaHoy(esquema: string): boolean {
...
export async function obtenerTurnoPorInscripcion(inscripcionId: string) {
...
export async function inactivarTurno(inscripcionId: string) {
...
export async function contadoresDia() {
  ...
  const programadosHoy = todosLosTurnos.filter((t) =>
    (DIAS_ESQUEMA[t.esquema] ?? []).includes(diaHoy)
  ).length;
...
export async function listarSerologia(inscripcionId: string) {
  ...
  const mapa: Record<string, (typeof registros)[0] | null> = {};
  for (const m of MARCADORES_HD) mapa[m] = null;
  for (const r of registros) mapa[r.marcador] = r;

  return { registros, mapa, marcadores: MARCADORES_HD };
}

export async function guardarResultadoSerologico(
  inscripcionId: string,
  pacienteId: string,
  data: {
    marcador: string;
...
export async function obtenerEstadoSerologico(inscripcionId: string) {
  const { mapa } = await listarSerologia(inscripcionId);

  // Determinar si el perfil está completo (todos tienen resultado no pendiente)
  const pendientes = Object.values(mapa).filter(
    (r) => !r || r.resultado === 'PENDIENTE'
  ).length;
