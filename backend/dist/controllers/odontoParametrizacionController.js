...
const toBool = (v, def = true) => (v === undefined || v === null ? def : !!v);
// ─────────────────────────────────────────────────────────────
//  HALLAZGOS
// ─────────────────────────────────────────────────────────────
export async function getHallazgos(req, res) {
    try {
        const incluirInactivos = norm(req.query.todos) === 'true';
...
export async function createHallazgo(req, res) {
    try {
...
export async function updateHallazgo(req, res) {
    try {
        const data = {};
        ...
        if (req.body.color !== undefined)
            data.color = norm(req.body.color);
        if (req.body.icono !== undefined)
            data.icono = norm(req.body.icono) || null;
        if (req.body.categoria !== undefined)
            data.categoria = norm(req.body.categoria) || null;
        if (req.body.generaTratamiento !== undefined)
            data.generaTratamiento = !!req.body.generaTratamiento;
        if (req.body.prioridadDefault !== undefined)
            data.prioridadDefault = norm(req.body.prioridadDefault) || null;
        if (req.body.orden !== undefined)
            data.orden = toInt(req.body.orden, 0);
...
export async function deleteHallazgo(req, res) {
    try {
...
export async function setSugerencias(req, res) {
    try {
        const hallazgoId = req.params.id;
        const cargoIds = Array.isArray(req.body.cargoIds) ? req.body.cargoIds : [];
...
function makeCatalog(model, extraFields = []) {
    const delegate = () => prisma[model];
    const list = async (req, res) => {
        try {
            const todos = norm(req.query.todos) === 'true';
