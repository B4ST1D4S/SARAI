...
export async function listarInscripcionesRenal(params: {
  ...
  if (!programa) throw new Error('Programa RENAL no configurado');
...
export async function inscribirPaciente(data: {
  ...
  if (existe) throw new Error('El paciente ya está inscrito activamente en el Programa Renal');

  const { historiaRenal, ...insc } = data;
  // Calcular riesgo KDIGO automáticamente
  const riesgoKDIGO = calcularRiesgoKDIGO(
    historiaRenal.estadioERC,
    historiaRenal.categoriaAlbuminuria
  );
...
export async function obtenerHistoriaRenal(inscripcionId: string) {
...
export async function listarAccesosVasculares(inscripcionId: string) {
...
export async function crearAccesoVascular(inscripcionId: string, data: any) {
  const inscripcion = await prisma.inscripcionPrograma.findUnique({
...
export async function actualizarAccesoVascular(id: string, data: any) {
...
export async function listarSesiones(
  inscripcionId: string,
  params: { skip?: number; take?: number; desde?: Date; hasta?: Date }
) {
...
export async function obtenerSesion(id: string) {
...
function sanearDatosSesion(data: any) {
  const clean: any = {};
  for (const key of Object.keys(data)) {
    if (!CAMPOS_SESION.has(key)) continue; // descartar campos desconocidos (bunPre, bunPost, etc.)
    const val = data[key];
...
export async function crearSesion(inscripcionId: string, data: any) {
  const inscripcion = await prisma.inscripcionPrograma.findUnique({
    where: { id: inscripcionId },
  });
  if (!inscripcion) throw new Error('Inscripción no encontrada');
...
export async function actualizarSesion(id: string, data: any) {
  const saneado = sanearDatosSesion(data);
...
export async function listarLaboratorios(
  inscripcionId: string,
  params: { skip?: number; take?: number; tipo?: string }
) {
  const { skip = 0, take = 20, tipo } = params;
  const where: any = { inscripcionId, ...(tipo ? { tipo } : {}) };
...
export async function crearLaboratorio(inscripcionId: string, data: any) {
  const inscripcion = await prisma.inscripcionPrograma.findUnique({
    where: { id: inscripcionId },
  });
  if (!inscripcion) throw new Error('Inscripción no encontrada');
...
export async function actualizarLaboratorio(id: string, data: any) {
  if (data.calcio && data.fosforo) {
    data.productoCaP = data.calcio * data.fosforo;
  }
...
export async function listarEventos(inscripcionId: string) {
...
export async function crearEvento(inscripcionId: string, data: any) {
  const inscripcion = await prisma.inscripcionPrograma.findUnique({
    where: { id: inscripcionId },
  });
  if (!inscripcion) throw new Error('Inscripción no encontrada');
...
export async function listarEvoluciones(
  inscripcionId: string,
  disciplina?: string
) {
...
export async function crearEvolucion(inscripcionId: string, data: any) {
  const inscripcion = await prisma.inscripcionPrograma.findUnique({
    where: { id: inscripcionId },
  });
  if (!inscripcion) throw new Error('Inscripción no encontrada');
...
export async function listarMaquinas(sedeId?: string) {
...
export async function crearMaquina(data: any) {
  return prisma.maquinaDialisis.create({ data });
}

export async function actualizarMaquina(id: string, data: any) {
  return prisma.maquinaDialisis.update({ where: { id }, data });
}

// ── Dashboard / KPIs ─────────────────────────────────────────

export async function getDashboardRenal(sedeId?: string) {
  const programa = await prisma.programaEspecial.findUnique({
    where: { codigo: 'RENAL' },
  });
...
export async function listarTamizajes(inscripcionId: string) {
...
export async function actualizarTamizaje(id: string, data: any) {
 ...
 */
function calcularRiesgoKDIGO(
  estadio?: string | null,
  albuminuria?: string | null
): string {
  if (!estadio || !albuminuria) return 'DESCONOCIDO';
 ...
 */
async function crearTamizajesInicialesHD(
  inscripcionId: string,
  pacienteId: string
) {
  const tamizajes = [
