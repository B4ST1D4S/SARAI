...
// ═══════════════════════════════════════════════════════
export async function resolverPermiso(usuarioId, recursoCodigo, accion, contexto) {
    ...
    if (!recurso)
        return { permitido: false, fuente: 'recurso_no_encontrado' };
    ...
    if (!usuario || !usuario.activo)
        return { permitido: false, fuente: 'usuario_inactivo' };
    ...
    const rolIds = usuario.UsuarioIamRol.map((r) => r.rolId);
    if (rolIds.length > 0) {
    ...
    const grupoIds = usuario.GrupoUsuario.map((g) => g.grupoId);
...
export const createEmpresa = async (req, res) => {
    try {
        ...
        if (e.code === 'P2002')
            return res.status(409).json({ error: 'NIT duplicado' });
    ...
    try {
        ...
        const perfil = await prisma.perfil.create({
            data: {
                ...
                updatedAt: new Date(),
                PerfilIamRol: rolesIds ? { create: rolesIds.map((id) => ({ rolId: id })) } : undefined,
            },
        });
...
export const deletePerfil = async (req, res) => {
    try {
        ...
        if (perfil?.esBase)
            return res.status(400).json({ error: 'No se puede eliminar un perfil base' });
...
//  IAM ROLES
// ═══════════════════════════════════════════════════════
export const getIamRoles = async (req, res) => {
    try {
...
export const createIamRol = async (req, res) => {
    try {
        ...
        if (e.code === 'P2002')
            return res.status(409).json({ error: 'Rol duplicado' });
        res.status(500).json({ error: 'Error al crear rol' });
    }
};
export const updateIamRol = async (req, res) => {
    try {
...
export const seedRecursosSistema = async (req, res) => {
    try {
        const recursos = [
            // MÓDULOS PRINCIPALES
...
export const getPermisos = async (req, res) => {
    try {
        ...
        if (sujetoTipo === 'USUARIO')
            where.usuarioId = sujetoId;
        if (sujetoTipo === 'PERFIL')
            where.perfilId = sujetoId;
        if (sujetoTipo === 'ROL')
            where.rolId = sujetoId;
        if (sujetoTipo === 'GRUPO')
            where.grupoId = sujetoId;
        if (recursoId)
...
export const setPermiso = async (req, res) => {
    try {
        ...
        if (!sujetoField)
            return res.status(400).json({ error: 'Tipo de sujeto inválido' });
...
export const checkPermiso = async (req, res) => {
    try {
        const { recursoCodigo, accion } = req.body;
        const usuarioId = req.user?.userId;
        if (!usuarioId)
            return res.status(401).json({ error: 'No autenticado' });
        const resultado = await resolverPermiso(usuarioId, recursoCodigo, accion);
        res.json(resultado);
    }
    catch (e) {
...
export const getMapaPermisos = async (req, res) => {
    try {
        const usuarioId = req.user?.userId;
        if (!usuarioId)
            return res.status(401).json({ error: 'No autenticado' });
        ...
        const [permisoPerfil, permisosRol, permisosGrupo, permisosDirectos] = await Promise.all([
            ...
            usuario.UsuarioIamRol.length > 0 ? prisma.permisoRecurso.findMany({
                where: { ...filtroActivo, rolId: { in: usuario.UsuarioIamRol.map((r) => r.rolId) } },
                include: { RecursoSistema: { select: { codigo: true } } },
            }) : [],
            usuario.GrupoUsuario.length > 0 ? prisma.permisoRecurso.findMany({
                where: { ...filtroActivo, grupoId: { in: usuario.GrupoUsuario.map((g) => g.grupoId) } },
                include: { RecursoSistema: { select: { codigo: true } } },
            }) : [],
            prisma.permisoRecurso.findMany({
        ...
        const denegar = new Set(denegarDirecto.map((p) => `${p.RecursoSistema.codigo}:${p.accion}`));
        const todos = [...permisoPerfil, ...permisosRol, ...permisosGrupo, ...permisosDirectos];
        const permitidos = todos
            .filter((p) => !denegar.has(`${p.RecursoSistema.codigo}:${p.accion}`))
            .map((p) => ({ recurso: p.RecursoSistema.codigo, accion: p.accion }));
        // Deduplicar
        const mapa = {};
        for (const p of permitidos) {
            if (!mapa[p.recurso])
                mapa[p.recurso] = new Set();
...
export const getMyPermissions = async (req, res) => {
    try {
        ...
        const acciones = ['VER', 'CREAR', 'EDITAR', 'ELIMINAR', 'IMPRIMIR', 'EXPORTAR', 'APROBAR', 'FIRMAR', 'ANULAR'];
        const mapa = {};
...
export const revokeDelegacion = async (req, res) => {
    try {
...
//  AUDITORÍA
// ═══════════════════════════════════════════════════════
export const getAuditAccesos = async (req, res) => {
    try {
        ...
        if (desde || hasta) {
            where.createdAt = {};
            if (desde)
                where.createdAt.gte = new Date(String(desde));
            if (hasta)
                where.createdAt.lte = new Date(String(hasta));
        }
...
export const getEventosSeguridad = async (req, res) => {
    try {
...
export const resolverEvento = async (req, res) => {
    try {
...
// ═══════════════════════════════════════════════════════
export const getDashboardIam = async (req, res) => {
    try {
        const [totalUsuarios, totalPerfiles, totalRoles, totalGrupos, sesionesActivas, eventosPendientes, delegacionesActivas, accesosHoy,] = await Promise.all([
