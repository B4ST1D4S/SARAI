...
export const FACTURA_SIN_CONTRATO_CODIGOS = {
...
function validarCucon(data) {
    if (data.tieneCucon === undefined)
        return; // no se está tocando este bloque
    if (data.tieneCucon) {
...
export async function createEmpresa(data) {
    ...
    if (existe)
        throw new Error(`Ya existe una empresa con NIT ${data.nit}`);
...
const CONTRATO_INCLUDE = {
    ...
    tarifas: { where: { activo: true }, orderBy: { codigoCUPS: 'asc' } },
    paquetes: {
        where: { activo: true },
        include: { items: { orderBy: { esPrincipal: 'desc' } } },
        orderBy: { nombre: 'asc' },
    },
...
export async function updateContrato(id, data) {
    ...
    if (data.tieneCucon !== undefined) {
        ...
        if (data.tieneCucon)
            data.facturaSinContrato = null;
        else
            data.codigoCucon = null;
    }
...
export async function cambiarEstadoContrato(id, estado, userId) {
    const contrato = await prisma.contrato.findUnique({ where: { id } });
    if (!contrato)
        throw new Error('Contrato no encontrado');
...
export async function deleteTarifa(contratoId, tarifaId) {
...
export async function getExcepciones(contratoId) {
...
export async function deleteExcepcion(contratoId, excepcionId) {
...
export async function clonarContrato(contratoOrigenId, data) {
    ...
    if (!origen)
        throw new Error('Contrato de origen no encontrado');
    const empresa = await prisma.empresaContratante.findUnique({ where: { id: data.empresaId } });
    if (!empresa)
        throw new Error('Empresa no encontrada');
...
// ─── BENEFICIARIOS ──────────────────────────────────────────────────────────
export async function getBeneficiarios(contratoId) {
...
export async function updateBeneficiario(id, data) {
    return prisma.contratoBeneficiario.update({ where: { id }, data });
}
export async function removeBeneficiario(id) {
