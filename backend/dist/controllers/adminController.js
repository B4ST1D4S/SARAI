...
export async function createEspecialidad(req, res) {
    try {
        const { nombre, descripcion, aplicaAnestesia, aplicaPediatria, aplicaCirugia, aplicaInstrumentacion, aplicaMedicoFamiliar } = req.body;
        if (!nombre)
...
export async function deleteEspecialidad(req, res) {
    try {
        ...
        if (enUso)
            return res.status(400).json({ error: 'Especialidad en uso. No se puede eliminar.' });
...
// 2. HC MÓDULOS
// ─────────────────────────────────────────
export async function getHCModulos(req, res) {
    try {
...
export async function createHCModulo(req, res) {
    try {
        ...
        const item = await prisma.hCModulo.create({
            data: { id: randomUUID(), codigo, nombre, descripcion, tipoFinalidad, tipoRips, parametrosConfiguracion, programaId,
                usuarioCreacion: req.user?.id, updatedAt: new Date() },
        });
        res.status(201).json(item);
    }
    catch {
...
export async function updateHCModulo(req, res) {
    try {
        const { id } = req.params;
        const data = req.body;
        delete data.id;
...
export async function deleteHCModulo(req, res) {
    try {
        const { id } = req.params;
        const enUso = await prisma.tipoConsulta.findFirst({ where: { hcModuloId: id } });
        if (enUso)
            return res.status(400).json({ error: 'Módulo HC en uso. No se puede eliminar.' });
...
export async function deleteDepartamento(req, res) {
    try {
        ...
        if (enUso)
            return res.status(400).json({ error: 'Departamento en uso.' });
...
export async function createServicio(req, res) {
    try {
        const { codigoCups, nombre, descripcion, categoria, subcategoria, tipoServicio, nivelComplejidad, conceptoRips, precioBase, requiereCantidad, esHonorario, esPOS } = req.body;
        if (!codigoCups || !nombre)
            return res.status(400).json({ error: 'codigoCups y nombre son requeridos' });
        const existe = await prisma.servicioFacturable.findUnique({ where: { codigoCups } });
        if (existe)
            return res.status(400).json({ error: 'Código CUPS ya existe' });
...
// 5. TIPOS DE CONSULTA
// ─────────────────────────────────────────
export async function getTiposConsulta(req, res) {
    try {
...
export async function getTipoConsultaById(req, res) {
    try {
        const { id } = req.params;
...
export async function createTipoConsulta(req, res) {
    try {
        const { nombre, descripcion, especialidadId, departamentoId, hcModuloId, requiereCaja, manejaAnestesia, permiteAgendamiento, controlaTiempoCita, abreHistoriaClinica, permiteCargosAdicionales, esProgramaPYP, manejaProtocolos, clasificacion, esPsicologia, duracionMinutos, bodegaId, servicios } = req.body;
        if (!nombre)
            return res.status(400).json({ error: 'nombre es requerido' });
...
export async function updateTipoConsulta(req, res) {
    try {
        const { id } = req.params;
        // Desestructurar solo campos escalares — descartar relaciones anidadas
        const { nombre, descripcion, especialidadId, departamentoId, hcModuloId, requiereCaja, manejaAnestesia, permiteAgendamiento, controlaTiempoCita, abreHistoriaClinica, permiteCargosAdicionales, esProgramaPYP, manejaProtocolos, clasificacion, esPsicologia, duracionMinutos, bodegaId, estado, servicios, } = req.body;
...
export async function deleteTipoConsulta(req, res) {
    try {
        const { id } = req.params;
...
// ─────────────────────────────────────────
export async function getConfigServicios(req, res) {
    try {
...
export async function addServicioAConsulta(req, res) {
    try {
        const { tipoConsultaId } = req.params;
        const { servicioId, esPrincipal, generaAutomatico, requiereOrden, centroOperacionId, cuentaContable, hcModuloId } = req.body;
        if (!servicioId)
        ...
        if (!existeSF) {
            ...
            if (!cups)
                return res.status(404).json({ error: 'Servicio / código CUPS no encontrado' });
...
export async function removeServicioDeConsulta(req, res) {
    try {
        const { id } = req.params;
...
// ─────────────────────────────────────────
export async function getReglasOperativas(req, res) {
    try {
...
export async function upsertReglaOperativa(req, res) {
    try {
        const { departamentoId } = req.params;
        const { servicioId, permiteSeleccion, manejaInsumos, generaOrden, liquidaHonorarios, cumplimientoAutomatico, tomadoAutomatico, cumplimientoParcial, manejaCentroCosto } = req.body;
        if (!servicioId)
            return res.status(400).json({ error: 'servicioId es requerido' });
...
// ─────────────────────────────────────────
export async function getPreparaciones(req, res) {
    try {
        const { tipoConsultaId, especialidadId } = req.query;
        const where = { estado: true };
        if (tipoConsultaId)
            where.tipoConsultaId = tipoConsultaId;
        if (especialidadId)
...
export async function createPreparacion(req, res) {
    try {
        ...
        if (existe)
            return res.status(409).json({ error: `Ya existe una preparación con el nombre "${nombre.trim()}". Verifique antes de guardar.` });
...
export async function updatePreparacion(req, res) {
    try {
        const { id } = req.params;
        const data = req.body;
        delete data.id;
...
export async function deletePreparacion(req, res) {
    try {
        const { id } = req.params;
...
export async function createCargo(req, res) {
    try {
        const { codigo, nombre, descripcion, tipo, valor, unidad, codigoReferencia, aplicaIva, tasaIva, esObligatorio, aplicaPYP } = req.body;
        if (!codigo || !nombre)
            return res.status(400).json({ error: 'codigo y nombre son requeridos' });
        const existe = await prisma.cargo.findFirst({ where: { codigo } });
        if (existe)
            return res.status(400).json({ error: 'Ya existe un cargo con ese código' });
        const item = await prisma.cargo.create({
            data: {
                ...
                usuarioCreacion: req.user?.userId,
                updatedAt: new Date(),
            },
        });
        res.status(201).json(item);
...
export async function updateCargo(req, res) {
    try {
        const { id } = req.params;
        const { codigo, nombre, descripcion, tipo, valor, unidad, codigoReferencia, aplicaIva, tasaIva, esObligatorio, aplicaPYP, estado } = req.body;
...
export async function bulkCreateCargos(req, res) {
    try {
        ...
        for (const row of items) {
            if (!row.codigo || !row.nombre) {
                results.errors.push(`Fila sin codigo/nombre: ${JSON.stringify(row)}`);
                continue;
            }
            ...
            await prisma.cargo.create({ data: { id: randomUUID(), codigo: row.codigo, nombre: row.nombre, descripcion: row.descripcion, tipo: row.tipo || 'CONSULTA', valor: Number(row.valor) || 0, unidad: row.unidad, codigoReferencia: row.codigoReferencia, usuarioCreacion: req.user?.userId, updatedAt: new Date() } });
            results.created++;
        }
        res.json(results);
    }
...
export async function bulkCreateEspecialidades(req, res) {
    try {
        const { items } = req.body;
        if (!Array.isArray(items) || items.length === 0)
        ...
        for (const row of items) {
            if (!row.codigo || !row.nombre) {
                results.errors.push(`Sin codigo/nombre: ${JSON.stringify(row)}`);
                continue;
            }
            ...
            await prisma.especialidad.create({ data: { codigo: row.codigo, nombre: row.nombre, descripcion: row.descripcion, usuarioCreacion: req.user?.userId, updatedAt: new Date() } });
            results.created++;
        }
        res.json(results);
    }
...
export async function bulkCreateDepartamentos(req, res) {
    try {
        const { items } = req.body;
        if (!Array.isArray(items) || items.length === 0)
            return res.status(400).json({ error: 'Se requiere arreglo items' });
...
export async function bulkCreateTiposConsulta(req, res) {
    try {
        const { items } = req.body;
        if (!Array.isArray(items) || items.length === 0)
            return res.status(400).json({ error: 'Se requiere arreglo items' });
        ...
        for (const row of items) {
            if (!row.nombre) {
                results.errors.push(`Sin nombre: ${JSON.stringify(row)}`);
                continue;
            }
            await prisma.tipoConsulta.create({ data: { id: randomUUID(), nombre: row.nombre, descripcion: row.descripcion, clasificacion: row.clasificacion || 'CONSULTA', duracionMinutos: Number(row.duracionMinutos) || 30, usuarioCreacion: req.user?.userId, updatedAt: new Date() } });
            results.created++;
        }
        res.json(results);
    }
...
// ─────────────────────────────────────────
export async function getTiposConsultorio(req, res) {
    try {
...
export async function createTipoConsultorio(req, res) {
    try {
        const { codigo, tipoConsultorio, descripcion, indiceAutomatico } = req.body;
        if (!codigo || !tipoConsultorio)
            return res.status(400).json({ error: 'codigo y tipoConsultorio son requeridos' });
...
export async function updateTipoConsultorio(req, res) {
    try {
        const { id } = req.params;
...
export async function deleteTipoConsultorio(req, res) {
    try {
        const { id } = req.params;
...
// ─────────────────────────────────────────
export async function getDepartamentoCargos(req, res) {
    try {
        const { departamentoId } = req.params;
...
export async function createDepartamentoCargo(req, res) {
    try {
        const { departamentoId } = req.params;
        const { cargoId, permiteSeleccion, manejaInsumos, cumplimientoAutomatico, tomadoAutomatico, interfaceExterno, generaOrden, liquidaHonorarios, cumplimientoParcial, manejaCentroCosto, } = req.body;
        if (!cargoId)
            return res.status(400).json({ error: 'cargoId es requerido' });
        const existe = await prisma.departamentoCargo.findFirst({ where: { departamentoId, cargoId } });
        if (existe)
            return res.status(400).json({ error: 'El cargo ya está asignado a este departamento' });
...
export async function updateDepartamentoCargo(req, res) {
    try {
        const { id } = req.params;
        const { permiteSeleccion, manejaInsumos, cumplimientoAutomatico, tomadoAutomatico, interfaceExterno, generaOrden, liquidaHonorarios, cumplimientoParcial, manejaCentroCosto, } = req.body;
...
export async function deleteDepartamentoCargo(req, res) {
    try {
        const { id } = req.params;
...
export async function getCamposPaciente(req, res) {
    try {
...
export async function createCampoPaciente(req, res) {
    try {
        const { nombre, etiqueta, seccion, tipoCampo, esObligatorio, opciones, placeholder, orden } = req.body;
        if (!nombre || !etiqueta || !seccion)
            return res.status(400).json({ error: 'nombre, etiqueta y seccion son requeridos' });
...
export async function updateCampoPaciente(req, res) {
    try {
        const { id } = req.params;
...
export async function deleteCampoPaciente(req, res) {
    try {
        const { id } = req.params;
        const campo = await prisma.campoPaciente.findUnique({ where: { id } });
        if (!campo)
            return res.status(404).json({ error: 'Campo no encontrado' });
        if (!campo.esPersonalizado)
            return res.status(400).json({ error: 'Solo se pueden eliminar campos personalizados' });
...
export async function resetCamposPaciente(req, res) {
    try {
...
async function seedParamGroup(grupo, defaults) {
...
export async function getParametrosSistema(req, res) {
    try {
        ...
        if (grupo === 'clinica') {
            ...
            const items = PARAMS_CLINICA.map((p) => ({
                ...
                grupo: 'clinica',
                clave: p.clave,
                valor: empresa[EMPRESA_FIELD_MAP[p.clave]] ?? '',
                etiqueta: p.etiqueta,
                tipo: p.tipo,
                orden: p.orden,
                estado: true,
        ...
        if (count === 0) {
            if (grupo === 'agenda')
                await seedParamGroup('agenda', PARAMS_AGENDA);
        }
...
export async function updateParametroSistema(req, res) {
    try {
        ...
        if (grupo === 'clinica') {
            ...
            res.json({ grupo, clave, valor: updated[field] ?? '', updatedAt: updated.updatedAt });
            return;
        }
        const now = new Date();
...
export async function getListasValores(req, res) {
    try {
...
export async function createListaValor(req, res) {
    try {
        const { grupo, valor, etiqueta, orden } = req.body;
        if (!grupo || !valor || !etiqueta)
            return res.status(400).json({ error: 'grupo, valor y etiqueta son requeridos' });
        const existe = await prisma.listaValor.findUnique({ where: { grupo_valor: { grupo, valor } } });
        if (existe)
            return res.status(400).json({ error: 'Ya existe ese valor en el grupo' });
...
export async function updateListaValor(req, res) {
    try {
        const { id } = req.params;
...
export async function deleteListaValor(req, res) {
    try {
        const { id } = req.params;
...
export async function getMotivosCita(req, res) {
    try {
...
export async function createMotivoCita(req, res) {
    try {
        ...
        if (existe)
            return res.status(409).json({ error: `Ya existe un motivo de cita con el nombre "${nombre.trim()}". Verifique antes de guardar.` });
...
export async function updateMotivoCita(req, res) {
    try {
        const { id } = req.params;
...
export async function deleteMotivoCita(req, res) {
    try {
        const { id } = req.params;
