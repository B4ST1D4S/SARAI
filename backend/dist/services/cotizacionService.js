...
// Enviar cotización por email
async function sendCotizacionEmail(cotizacion) {
    try {
        const htmlTemplate = `
      <!DOCTYPE html>
      <html>
...
export async function getCotizacionesPaciente(pacienteId) {
    try {
...
// Rechazar cotización
export async function rechazarCotizacion(id, motivo) {
    try {
        const cotizacion = await prisma.cotizacion.update({
            where: { id },
            data: {
...
export async function getCotizacionesMedico(medicoId) {
    try {
        const cotizaciones = await prisma.cotizacion.findMany({
            where: { medicoId },
            include: {
