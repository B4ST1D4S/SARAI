...
export async function getLeads(req, res) {
    try {
        const { etapa, calificacion, search, limit = '100', offset = '0' } = req.query;
        const where = {};
...
export async function getStats(req, res) {
    try {
        ...
        const [totalLeads, leadsEstesMes, convertidos, perdidos, porEtapa, porCalificacion, valorPipeline, seguimientosHoy,] = await Promise.all([
...
export async function createLead(req, res) {
    try {
        ...
        const { nombre, telefono, email, procedimientos, etapa, calificacion, valorEstimado, origen, notas, observaciones, tags, pacienteId, proximoContacto, } = req.body;
        if (!nombre) {
...
export async function updateLead(req, res) {
    try {
        ...
        const data = { ultimaInteraccion: new Date() };
        if (nombre !== undefined)
            data.nombre = nombre;
        ...
        if (valorEstimado !== undefined)
            data.valorEstimado = parseFloat(valorEstimado);
        if (origen !== undefined)
            data.origen = origen;
        if (notas !== undefined)
            data.notas = notas;
        ...
        if (proximoContacto !== undefined)
            data.proximoContacto = proximoContacto ? new Date(proximoContacto) : null;
...
export async function syncLeads(req, res) {
    try {
        ...
        for (const cot of cotizaciones) {
            if (!byPaciente.has(cot.pacienteId))
                byPaciente.set(cot.pacienteId, cot);
        }
        let created = 0;
        let skipped = 0;
