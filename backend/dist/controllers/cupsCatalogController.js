 ...
 */
function derivarCups(codigoRaw) {
    ...
    if (nivel === 'SUBCATEGORIA')
        parentCodigo = d.slice(0, 4);
    else if (nivel === 'CATEGORIA')
        parentCodigo = d.slice(0, 3);
    else if (nivel === 'SUBGRUPO')
        parentCodigo = d.slice(0, 2);
    return {
        codigo: d,
        codigoFormato,
        nivel,
...
export async function getCupsCodigos(req, res) {
    try {
        const { search, nivel, capitulo, activo, esFacturable } = req.query;
        ...
        if (esFacturable !== undefined && esFacturable !== '')
            where.esFacturable = esFacturable === 'true';
...
export async function getCupsCodigosStats(_req, res) {
    try {
...
export async function createCupsCodigo(req, res) {
    try {
        ...
        if (existe)
            return res.status(400).json({ error: `El código ${der.codigoFormato} ya existe` });
        let parentId = null;
        if (der.parentCodigo) {
...
export async function updateCupsCodigo(req, res) {
    try {
        const { id } = req.params;
...
// ─── Eliminar (desactivar) ────────────────────────────────────
export async function deleteCupsCodigo(req, res) {
    try {
        const { id } = req.params;
...
export async function bulkCreateCupsCodigos(req, res) {
    try {
        ...
        for (const row of items) {
            ...
            if (!codigo || !descripcion) {
                errors.push(`Fila sin código/descripción: ${JSON.stringify(row)}`);
                continue;
            }
            const der = derivarCups(codigo);
            if (!der) {
                errors.push(`Código inválido: ${codigo}`);
                continue;
            }
            derivadas.push({
                ...der,
