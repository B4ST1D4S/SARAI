import { createHistoriaClinica, getHistoriaClinicaById, getHistoriasPaciente, updateHistoriaClinica, getHistoriasPorMedico, entregarHistoriaClinica, } from '../services/historiaClinicaService.js';
export async function create(req, res) {
    try {
        ...
        const { pacienteId, tipoHistoria, tipoConsulta, quejaPrincipal, historiaEnfermedad, observacionesAntropometricas, diagnostico, tratamientoRecomendado, fotos, datosExtendidos, } = req.body;
...
export async function getPorPaciente(req, res) {
    try {
...
export async function getPorMedico(req, res) {
    try {
        if (!req.user) {
            res.status(401).json({ error: 'No autenticado' });
            return;
...
export async function update(req, res) {
    try {
        ...
        const { id } = req.params;
        const { tipoConsulta, tipoHistoria, quejaPrincipal, historiaEnfermedad, observacionesAntropometricas, diagnostico, tratamientoRecomendado, datosExtendidos, } = req.body;
        if (!id) {
            res.status(400).json({ error: 'ID requerido' });
            return;
        }
