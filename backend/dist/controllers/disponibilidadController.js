...
import { getDisponibilidadMedico, createDisponibilidad, updateDisponibilidad, deleteDisponibilidad, getBloqueos, createBloqueo, deleteBloqueo, getSlotsConEstado, getDisponibilidadesConCitas, getMedicosPorTipoConsulta, getDiasDisponibles, } from '../services/disponibilidadService.js';
// GET /api/disponibilidad/medico/:medicoId
export async function getMedicoDisponibilidad(req, res) {
    try {
...
export async function postDisponibilidad(req, res) {
    try {
        if (!req.user) {
...
export async function getMedicosList(req, res) {
    try {
...
export async function getTiposConsultaMedico(req, res) {
    try {
        const { medicoId } = req.params;
...
export async function putDisponibilidad(req, res) {
    try {
...
export async function deleteDisponibilidadCtrl(req, res) {
    try {
...
export async function getDisponibilidadesConCitasCtrl(req, res) {
    try {
...
export async function getSlots(req, res) {
    try {
        const { medicoId, fecha, duracion } = req.query;
        if (!medicoId || !fecha) {
...
export async function getBloqueosMedico(req, res) {
    try {
...
export async function postBloqueo(req, res) {
    try {
        if (!req.user) {
            res.status(401).json({ error: 'No autenticado' });
            return;
...
export async function deleteBloqueoCtrl(req, res) {
    try {
...
export async function getMedicosPorTipoCtrl(req, res) {
    try {
        const { tipoConsultaNombre } = req.query;
...
export async function getDiasDisponiblesCtrl(req, res) {
    try {
        const { medicoId, mes, anio, duracion } = req.query;
        if (!medicoId || !mes || !anio) {
            res.status(400).json({ error: 'medicoId, mes y anio son requeridos' });
            return;
        }
        const dias = await getDiasDisponibles(medicoId, parseInt(mes, 10), parseInt(anio, 10), duracion ? parseInt(duracion, 10) : undefined);
