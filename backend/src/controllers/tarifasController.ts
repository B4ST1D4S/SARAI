...
const digits = (s?: string | null) => norm(s).replace(/\D/g, '');

// ─────────────────────────────────────────────────────────────
//  CLASIFICACIÓN — Grupos y Tipos de cargo
// ─────────────────────────────────────────────────────────────

export async function getTarifaGrupos(_req: Request, res: Response) {
  try {
...
export async function createTarifaGrupo(req: Request, res: Response) {
  try {
    ...
    if (existe) return res.status(409).json({ error: 'Ya existe un grupo con ese código' });
...
export async function updateTarifaGrupo(req: Request, res: Response) {
  try {
    const { id } = req.params;
    const data: any = {};
...
export async function deleteTarifaGrupo(req: Request, res: Response) {
  try {
    const { id } = req.params;
...
export async function createTarifaTipo(req: Request, res: Response) {
  try {
    const grupoId = norm(req.body.grupoId);
    const codigo = norm(req.body.codigo);
    const nombre = norm(req.body.nombre);
    if (!grupoId) return res.status(400).json({ error: 'El grupo es requerido' });
    if (!codigo) return res.status(400).json({ error: 'El código es requerido' });
    if (!nombre) return res.status(400).json({ error: 'El nombre es requerido' });
    const existe = await prisma.tarifaTipo.findUnique({ where: { grupoId_codigo: { grupoId, codigo } } });
    if (existe) return res.status(409).json({ error: 'Ya existe un tipo con ese código en el grupo' });
...
export async function updateTarifaTipo(req: Request, res: Response) {
  try {
    const { id } = req.params;
    const data: any = {};
    if (req.body.nombre !== undefined) data.nombre = norm(req.body.nombre);
...
export async function deleteTarifaTipo(req: Request, res: Response) {
  try {
    const { id } = req.params;
...
async function resolverCupsId(codigoCups?: string | null): Promise<{ id: string | null; codigo: string | null }> {
  const d = digits(codigoCups);
  if (!d) return { id: null, codigo: null };
...
export async function getCargosTarifa(req: Request, res: Response) {
  try {
    const { search, grupoId, tipoId, activo } = req.query as Record<string, string>;
...
export async function getCargosTarifaStats(_req: Request, res: Response) {
  try {
...
export async function createCargoTarifa(req: Request, res: Response) {
  try {
    const codigo = norm(req.body.codigo);
    ...
    if (existe) return res.status(409).json({ error: 'Ya existe un cargo con ese código' });
...
export async function updateCargoTarifa(req: Request, res: Response) {
  try {
    const { id } = req.params;
    const data: any = {};
    if (req.body.descripcion !== undefined) data.descripcion = norm(req.body.descripcion);
    if (req.body.grupoId !== undefined) data.grupoId = norm(req.body.grupoId) || null;
    if (req.body.tipoId !== undefined) data.tipoId = norm(req.body.tipoId) || null;
    if (req.body.nivel !== undefined) data.nivel = norm(req.body.nivel) || null;
    if (req.body.tipoUnidad !== undefined) data.tipoUnidad = norm(req.body.tipoUnidad) || null;
    if (req.body.conceptoRips !== undefined) data.conceptoRips = norm(req.body.conceptoRips) || null;
    if (req.body.activo !== undefined) data.activo = !!req.body.activo;
...
export async function deleteCargoTarifa(req: Request, res: Response) {
  try {
    const { id } = req.params;
...
export async function bulkCreateCargosTarifa(req: Request, res: Response) {
  try {
    ...
    const results = { created: 0, skipped: 0, errors: [] as { fila: number; error: string }[] };
...
export async function getTarifarios(req: Request, res: Response) {
  try {
    const { search, activo } = req.query as Record<string, string>;
    const where: any = {};
    if (search?.trim()) {
      where.OR = [
...
export async function getTarifarioById(req: Request, res: Response) {
  try {
    const { id } = req.params;
    ...
    if (!tarifario) return res.status(404).json({ error: 'Tarifario no encontrado' });
    res.json(tarifario);
  } catch {
...
export async function createTarifario(req: Request, res: Response) {
  try {
    const codigo = norm(req.body.codigo);
    const nombre = norm(req.body.nombre);
    if (!codigo) return res.status(400).json({ error: 'El código es requerido' });
    ...
    if (existe) return res.status(409).json({ error: 'Ya existe un tarifario con ese código' });
    ...
    if (porcentaje !== null && !Number.isFinite(porcentaje)) return res.status(400).json({ error: 'El porcentaje no es válido' });
...
export async function updateTarifario(req: Request, res: Response) {
  try {
    const { id } = req.params;
    const data: any = {};
    if (req.body.nombre !== undefined) data.nombre = norm(req.body.nombre);
    if (req.body.descripcion !== undefined) data.descripcion = norm(req.body.descripcion) || null;
    if (req.body.tipo !== undefined) data.tipo = norm(req.body.tipo) || null;
    if (req.body.baseId !== undefined) data.baseId = norm(req.body.baseId) || null;
    if (req.body.porcentaje !== undefined) data.porcentaje = req.body.porcentaje === '' ? null : Number(req.body.porcentaje);
    if (req.body.vigenciaDesde !== undefined) data.vigenciaDesde = req.body.vigenciaDesde ? new Date(req.body.vigenciaDesde) : null;
    if (req.body.vigenciaHasta !== undefined) data.vigenciaHasta = req.body.vigenciaHasta ? new Date(req.body.vigenciaHasta) : null;
    if (req.body.activo !== undefined) data.activo = !!req.body.activo;
...
export async function deleteTarifario(req: Request, res: Response) {
  try {
    const { id } = req.params;
 ...
 */
export async function generarTarifarioDesdeBase(req: Request, res: Response) {
  try {
    const { id } = req.params;
    const tarifario = await prisma.tarifario.findUnique({ where: { id } });
    if (!tarifario) return res.status(404).json({ error: 'Tarifario no encontrado' });
    if (!tarifario.baseId) return res.status(400).json({ error: 'El tarifario no tiene un tarifario base configurado' });
...
export async function getTarifarioItems(req: Request, res: Response) {
  try {
    ...
    const where: any = { tarifarioId: id };
    if (activo !== undefined && activo !== '') where.activo = activo === 'true';
    if (search?.trim()) {
      const raw = search.trim();
      const d = raw.replace(/\D/g, '');
...
export async function upsertTarifarioItem(req: Request, res: Response) {
  try {
    const { id } = req.params; // tarifarioId
    const cargoId = norm(req.body.cargoId);
    const precio = Number(req.body.precio);
    if (!cargoId) return res.status(400).json({ error: 'El cargo es requerido' });
    if (!Number.isFinite(precio) || precio < 0) return res.status(400).json({ error: 'El precio no es válido' });
...
export async function deleteTarifarioItem(req: Request, res: Response) {
  try {
...
export async function bulkTarifarioItems(req: Request, res: Response) {
  try {
    const { id } = req.params; // tarifarioId
    const rows: any[] = Array.isArray(req.body.items) ? req.body.items : [];
    if (!rows.length) return res.status(400).json({ error: 'No se recibieron registros' });
    ...
    const results = { created: 0, updated: 0, errors: [] as { fila: number; error: string }[] };

    for (let i = 0; i < rows.length; i++) {
      const row = rows[i];
      const codCargo = norm(row.cargo || row.codigo);
      const precio = Number(row.precio);
      if (!codCargo) { results.errors.push({ fila: i + 1, error: 'falta el código del cargo' }); continue; }
      if (!Number.isFinite(precio) || precio < 0) { results.errors.push({ fila: i + 1, error: 'precio inválido' }); continue; }
      const cargo = await prisma.tarifaCargo.findUnique({ where: { codigo: codCargo }, select: { id: true } });
      if (!cargo) { results.errors.push({ fila: i + 1, error: `cargo "${codCargo}" no existe` }); continue; }
      ...
      if (existe) results.updated++; else results.created++;
    }
    res.json(results);
  } catch {
