...
async function resolverMedicoId(req) {
...
async function audit(req, tabla, registroId, operacion, antes, despues) {
    try {
...
// ─────────────────────────────────────────────────────────────
export async function getCatalogos(_req, res) {
    try {
...
export async function buscarCargos(req, res) {
    try {
...
//  ODONTOGRAMAS
// ─────────────────────────────────────────────────────────────
export async function getOdontogramasByPaciente(req, res) {
    try {
        const tipo = norm(req.query.tipo);
        const where = { pacienteId: req.params.pacienteId };
        if (tipo)
            where.tipo = tipo;
...
export async function getOdontogramaById(req, res) {
    try {
...
export async function crearOdontograma(req, res) {
    try {
...
export async function updateOdontograma(req, res) {
    try {
        const data = {};
        if (req.body.hallazgosGenerales !== undefined)
            data.hallazgosGenerales = req.body.hallazgosGenerales;
        if (req.body.estetica !== undefined)
            data.estetica = req.body.estetica;
        if (req.body.resumenIA !== undefined)
            data.resumenIA = norm(req.body.resumenIA) || null;
        if (req.body.riesgoId !== undefined)
            data.riesgoId = norm(req.body.riesgoId) || null;
        if (req.body.denticion !== undefined)
            data.denticion = norm(req.body.denticion);
        if (req.body.estado !== undefined)
            data.estado = norm(req.body.estado);
        if (req.body.firmado !== undefined)
            data.firmado = !!req.body.firmado;
...
// ─────────────────────────────────────────────────────────────
export async function guardarPiezas(req, res) {
    try {
...
export async function generarPlan(req, res) {
    try {
        const odontogramaId = req.params.id;
        const odontograma = await prisma.odontograma.findUnique({
        ...
        for (const pieza of odontograma.piezas) {
            ...
            if (yaExiste)
                continue;
            const sugerencia = h.sugerencias?.find((s) => s.porDefecto) || h.sugerencias?.[0];
...
export async function addPlanItem(req, res) {
    try {
        const odontogramaId = req.params.id;
        const odontograma = await prisma.odontograma.findUnique({ where: { id: odontogramaId } });
        if (!odontograma) {
        ...
        if (cargoId) {
            ...
            if (cargo) {
                if (!descripcionProcedimiento)
                    descripcionProcedimiento = cargo.descripcion;
                if (!codigoCups)
                    codigoCups = cargo.cupsCodigoStr || cargo.codigo;
                if (!Number.isFinite(precio))
                    precio = cargo.items.length ? cargo.items[0].precio : 0;
            }
        }
        if (!Number.isFinite(precio))
            precio = 0;
        if (!descripcionProcedimiento)
            descripcionProcedimiento = diagnostico;
...
export async function updatePlanItem(req, res) {
    try {
        ...
        if (req.body.diagnostico !== undefined)
            data.diagnostico = norm(req.body.diagnostico);
        if (req.body.descripcionProcedimiento !== undefined)
            data.descripcionProcedimiento = norm(req.body.descripcionProcedimiento);
        if (req.body.codigoCups !== undefined)
            data.codigoCups = norm(req.body.codigoCups) || null;
        if (req.body.prioridadId !== undefined)
            data.prioridadId = norm(req.body.prioridadId) || null;
        if (req.body.observaciones !== undefined)
            data.observaciones = norm(req.body.observaciones) || null;
        if (req.body.evolucion !== undefined)
            data.evolucion = norm(req.body.evolucion) || null;
        if (req.body.diente !== undefined)
            data.diente = toIntOrNull(req.body.diente);
        if (req.body.superficie !== undefined)
            data.superficie = norm(req.body.superficie) || null;
        if (req.body.precio !== undefined)
            data.precio = toNum(req.body.precio, 0);
        if (req.body.fechaProgramada !== undefined)
            data.fechaProgramada = req.body.fechaProgramada ? new Date(req.body.fechaProgramada) : null;
        if (req.body.cargoId !== undefined) {
            ...
            if (cargoId) {
                ...
                if (cargo) {
                    ...
                    if (req.body.precio === undefined)
                        data.precio = cargo.items.length ? cargo.items[0].precio : 0;
                    if (req.body.descripcionProcedimiento === undefined)
                        data.descripcionProcedimiento = cargo.descripcion;
                }
            }
        }
...
// ─────────────────────────────────────────────────────────────
async function asegurarCuentaAbierta(pacienteId, citaId, medicoId) {
...
// ─────────────────────────────────────────────────────────────
export async function cambiarEstadoTratamiento(req, res) {
    try {
        ...
        const data = { estadoTratamiento: nuevoEstado };
        if (req.body.evolucion !== undefined)
            data.evolucion = norm(req.body.evolucion) || null;
        ...
        if (nuevoEstado === 'FINALIZADO') {
            ...
            if (item.cargoId && !item.facturado) {
                const cuenta = await asegurarCuentaAbierta(item.pacienteId, item.odontograma.citaId, await resolverMedicoId(req));
                const cantidad = 1;
        ...
        await prisma.odontoEvolucion.create({
            data: {
                ...
                descripcion: norm(req.body.evolucion) ||
...
// ─────────────────────────────────────────────────────────────
export async function addEvolucion(req, res) {
    try {
        const odontogramaId = req.params.id;
        const odontograma = await prisma.odontograma.findUnique({ where: { id: odontogramaId } });
        if (!odontograma) {
...
//  RESUMEN / KPIs
// ─────────────────────────────────────────────────────────────
export async function getResumen(_req, res) {
    try {
